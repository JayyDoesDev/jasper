...
export const NotifyVideoDiscussionsSubCommand = defineSubCommand({
  ...
  handler: async (ctx: Context, interaction: ChatInputCommandInteraction) => {
    try {
      ...
      if (!channel) {
        await interaction.reply({ content: "Could not find the video discussions channel.", flags: MessageFlags.Ephemeral });
        return;
      }
      ...
      if (currentVideoId === latest.id) {
        await interaction.reply({ content: "It looks like the most recent video has already been posted.", flags: MessageFlags.Ephemeral });
        return;
      }

      const messages = await channel.messages.fetch({ limit: 100 });
      if (messages.some(message => message.content.includes(latest.id))) {
        await interaction.reply({ content: "Latest video already posted.", flags: MessageFlags.Ephemeral });
        return;
      }
      ...
      await interaction.reply({ content: "Latest video now posted.", flags: MessageFlags.Ephemeral });
    } catch (error) {

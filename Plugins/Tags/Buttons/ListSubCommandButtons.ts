        ...
        on: (interaction: ButtonInteraction, ctx: Context) => {
            ...
            const embed = {
                embeds: [{
                    ...
                    color,
                    description: '',
                    footer: { text: `Page: 1/${currentUserState.tagPages.length} • emojis by AnThOnY & deussa` }
                }]
            };

            const updateEmbed = () => {
                embed.embeds[0].description = currentUserState.tagPages[currentUserState.page].map((e, i) => 
            ...
            if (interaction.isButton()) {
                switch (interaction.customId) {
                    ...
                    case `list_subcommand_button_next_${author}`:
                        currentUserState.page = (currentUserState.page + 1) % currentUserState.tagPages.length;
                        break;

                    case `list_subcommand_button_previous_${author}`:
                        currentUserState.page = (currentUserState.page - 1 + currentUserState.tagPages.length) % currentUserState.tagPages.length;
                        break;
                }

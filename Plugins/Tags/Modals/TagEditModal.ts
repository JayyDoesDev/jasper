            ...
            try {
                if (interaction.customId === `tag_edit_${interaction.user.id}`) {
                    ...
                    const description =
                        ...
                        null;
                    const image_url =
                        interaction.fields.getTextInputValue('tag_edit_embed_image_url').trim() ||
                        null;
                    const footer =
                        interaction.fields.getTextInputValue('tag_edit_embed_footer').trim() ||
                        null;
                    ...
                    const { TagEmbedTitle, TagEmbedDescription, TagEmbedImageURL, TagEmbedFooter } =
                        await ctx.services.tags.getValues<Options, TagResponse>({ guildId, name });

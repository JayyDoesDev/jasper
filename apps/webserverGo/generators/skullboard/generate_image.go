...
func calculateWidthHeight(font font.Face, data MessageData) (int, int, error) {
    ...
    // Dynamically calculate message block width based on content
    ...
    // Calculate required width for message content
    ...
    if data.RoleIconURL != "" {
        roleIconWidth, _ := tempDC.MeasureString("RoleIcon") // Placeholder for role icon width
        usernameLength += roleIconWidth + 10 // Add space for role icon
    }
    ...
    if data.ReplyContent != "" {
        contentHeight += 30
        if width < 300 {
            width = 300 // Ensure minimum width for reply block
        }
    }

    // Adjust width for attachments if present
...
func GenerateDiscordMessage(data MessageData) (image.Image, error) {
	...
	if data.ReplyContent != "" {
		...
		// og image is 138x54, resize for 40px height
		...
		replyAvatar, err := utils.LoadImageFromURL(data.ReplyAvatarURL)
		if err != nil {
			slog.Error("Failed to load reply avatar image", "url", data.ReplyAvatarURL, "error", err)
			return nil, err
		}
		replyAvatar = utils.ResizeImage(replyAvatar, 30, 30)
		// Draw rounded (circular) avatar
		...
		dc.ResetClip()
		currentX += 35
		currentY += 10 // Move down for reply block
		...
		currentY += 30 // Move down for next block
	}

	// Draw PFP
	pfp, err := utils.LoadImageFromURL(data.AvatarURL)
	if err != nil {
		slog.Error("Failed to load avatar image", "url", data.AvatarURL, "error", err)
		return nil, err
	}
	...
	if len(data.Attachments) > 0 {
		for _, attachmentURL := range data.Attachments {
			...
			attachmentY := int(currentY) // 20px padding below message
			dc.DrawImage(attachmentImage, int(messageBoxX), attachmentY)
			currentY += float64(attachmentImage.Bounds().Dy()) + 20 // Add attachment height + padding
		}
	}

...
export async function TagEdit(guildId: Snowflake, options: TagEditOptions, ctx: Context): Promise<void> {
  ...
  const tagIndex: number = cachedTags.findIndex((t) => t.TagName === options.name);
  if (tagIndex !== -1) {
    ...
    cachedTags[tagIndex] = {
      ...
      TagEmbedDescription: options.description ?? originalTag.TagEmbedDescription,
  ...
  await TagSchema.findOneAndUpdate(
    ...
    {
      $set: {
        "Tags.$.TagResponse.TagEmbedTitle": options.title ?? cachedTags[tagIndex].TagEmbedTitle,
        "Tags.$.TagResponse.TagEmbedDescription": options.description ?? cachedTags[tagIndex].TagEmbedDescription,
        "Tags.$.TagResponse.TagEmbedFooter": options.footer ?? cachedTags[tagIndex].TagEmbedFooter
      }
    }
  );
}
